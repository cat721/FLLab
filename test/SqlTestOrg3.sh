...
echo "POST invoke chaincode on peers of Org3 for the first time"
echo
...
echo "POST invoke chaincode on peers of Org3 for the second time"
echo
TRX_ID=$(curl -s -X POST \
  http://localhost:4002/channels/mychannel/chaincodes/mycc \
  -H "authorization: Bearer $ORG3_TOKEN" \
...
echo "POST invoke chaincode on peers of Org3 for the third time"
echo
TRX_ID=$(curl -s -X POST \
  http://localhost:4002/channels/mychannel/chaincodes/mycc \
  -H "authorization: Bearer $ORG3_TOKEN" \
