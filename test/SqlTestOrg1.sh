...
echo "POST invoke chaincode on peers of Org1 for the first time"
echo
...
echo "POST invoke chaincode on peers of Org1 for the second time"
echo
TRX_ID=$(curl -s -X POST \
  http://localhost:4000/channels/mychannel/chaincodes/mycc \
  -H "authorization: Bearer $ORG1_TOKEN" \
...
echo "POST invoke chaincode on peers of Org1 for the third time"
echo
TRX_ID=$(curl -s -X POST \
  http://localhost:4000/channels/mychannel/chaincodes/mycc \
  -H "authorization: Bearer $ORG1_TOKEN" \
